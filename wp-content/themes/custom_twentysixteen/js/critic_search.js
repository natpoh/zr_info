...
critic_search.init = function ($custom_id = '') {
    ...
    // Filters close
    $('#search-filters .filter .close:not(.init)').each(function (i, v) {
        var $this = $(v);
        $this.addClass('init');
        $this.click(function () {
            var $this_click = $(this);
            var $parent = $this_click.closest('.filter');
    ...
    // Filter menu
    $('#search-filters .filter .title:not(.init)').each(function (i, v) {
        var $this = $(v);
        $this.addClass('init');
        $this.click(function () {
            var $this_click = $(this);
            var $parent = $this_click.closest('.filter');
            if ($parent.hasClass('clear-all')) {
...
critic_search.init_collapse = function (id) {
    var cc = 'collapsed';
    var facet_id = id.split('-')[1];
    $('#' + id + ' > .facet-title.wacc:not(.init)').each(function () {
        var $this = $(this);
        $this.addClass('init');
        $this.click(function () {
            var v = $('#' + id);
    ...
    // Child collapse
    $('#' + id + ' .row_dd.acc:not(.init)').each(function () {
        var $this = $(this);
        $this.addClass('init');
        $this.click(function () {

            // row
            var facet_id_child = $this.closest('.with_dd').attr('data-dd');
            var id_child = 'row-' + facet_id_child;
            var v = $('#' + id_child);
...
critic_search.slider_facet = function (type, data_arr, ftype = 'all') {
    ...
    html5Slider.noUiSlider.on('set.one', function () {
        ...
        if (shift !== 0) {
            title = critic_search.formatNumber(title_f) + '-' + critic_search.formatNumber(title_t);
        }
...
critic_search.add_clear_all = function () {
    ...
    if (filter_len >= 1 && !$(".filters-wrapper .clear-all").length) {
        $('.filters-wrapper .save-filters').before('<li class="filter clear-all" title="Reset all search filters"><a href="' + clear_url + '"><span class="title clear">Reset filters</span></a></li>');
    }
};
...
critic_search.create_thumbnail = function (file, thumbnailSize = 400) {
    ...
    reader.onload = function (e) {
        ...
        img.onload = function () {
            ...
            var newHeight = thumbnailSize / aspectRatio;
            ...
            $('#filter_image').html('<img src="' + canvas.toDataURL('image/jpeg') + '">');
            $('#upl_filter_thumb').val(canvas.toDataURL('image/jpeg'));

...
class CriticSearch extends AbstractDB {
    ...
    public $facet_data = array(
        ...
        'ratings' => array(
            ...
            'childs' => array(
                ...
                'rmg' => array('title' => 'Metacritic % Gap', 'facet' => 'rating', 'eid' => 'emg', 'titlesm' => 'Metacritic % Gap', 'max_count' => 200, 'shift' => -10, 'multipler' => 10, 'group' => 'rating', 'icon' => 'mtcr', 'sorted' => 1, 'tabs' => array('movies', 'games', 'international', 'ethnicity'), 'hide' => 1, 'minus' => 1, 'zero' => 1, 'max' => 1),
        ...
        'wokedata' => array(
            ...
            'childs' => array(
                ...
                'rcherry' => array('title' => 'CherryPicks', 'facet' => 'rating', 'eid' => 'echerry', 'titlesm' => 'CherryPicks', 'max_count' => 100, 'multipler' => 1, 'group' => 'woke', 'sorted' => 1, 'hide' => 1, 'minus' => 1, 'tabs' => array('movies', 'international', 'ethnicity'), 'zero' => 1, 'max' => 1, 'sort_w' => 40,),
    ...
    public $search_filters = array(
        ...
        'kmwoke' => array(
            'woke' => array('key' => 'ewoke', 'title' => 'Possibly Woke'),
            'lgbt' => array('key' => 'elgbt', 'title' => 'LGBTQ'),
            'lgb' => array('key' => 'elgb', 'title' => 'LGB'),
            'qtia' => array('key' => 'eqtia', 'title' => 'QTIA+'),
        ),
    );
    // Actors cache
    ...
    public function get_filters_query($filters = array(), $exlude = array(), $query_type = 'movies', $curr_filter = '', $aid = 0) {
        ...
        if ($query_type == 'filters') {
            ...
            $select_and .= ", IF((publish AND items>0) OR aid={$aid},1,0) as upub";
            $filters_and .= " AND upub=1";
        } else {
            ...
            foreach ($filters as $key_data => $value) {
                ...
                if (isset($curr_facet['facet']) && $curr_facet['facet'] == 'rating') {
                    if ($value == 'use' || $value == 'minus') {
                        ...
                        if ($facet == 'rmg') {
                            $filters_and .= " AND rmu>0 AND rmc>0";
                        }
                    }
                } else if ($key == 'genre') {
                    ...
                    if ($key == 'kmwoke') {
                        ...
                        foreach ($value_loc as $kmkey) {
                            ...
                            if ($minus) {
                                $filters_and .= " AND " . $kfilter . "=0";
                            } else {
                                $filters_and .= " AND " . $kfilter . "=1";
                            }
                        }
                    } else {
                        $filters_and .= $this->filter_multi_value($key, $value, true, $minus);
                    ...
                    if ($actors_facet) {
                        ...
                        if ($cparent == 'race') {
                            ...
                            if ($key == 'sphoto' || $key == 'mphoto' || $key == 'aphoto') {
                                ...
                                $filters_and .= $this->filter_multi_value($key, $value, false, $minus);
                            } else {
                                // other

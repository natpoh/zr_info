<h2><a href="<?php print $url ?>"><?php print __('Movies Links Parser') ?></a>. <?php print __('Trash') ?></h2>
...
if ($cid) {
    ...
    <br />
    <h3>Remove all error posts</h3>
    <form accept-charset="UTF-8" method="post" id="campaign">
        <div class="cm-edit inline-edit-row">
            <fieldset>
                <input type="hidden" name="id" value="<?php print $cid ?>">
                <input type="hidden" name="remove_all_error_posts" value="1" >                
                <?php wp_nonce_field('ml-nonce', 'ml-nonce'); ?>
                
                <input type="submit" name="options" id="edit-submit" value="Remove all error posts" class="button">  
            </fieldset>
        </div>
    </form>
    <br />

...
class TMDB
{
    ...
    public static function get_next_update($mid,$rating_update=  array( 50=> 86400*7, 40 =>86400*14, 30=> 86400*30 , 20=> 86400*60, 10=> 86400*120, 0=>86400*300))
    {

        $q= "SEECT `weight`, `weight_upd` FROM data_movie_imdb where id = ".intval($mid);
        $r = Pdo_an::db_results_array($q);
        $weight = $r[0]['weight'];
        $weight_upd = $r[0]['weight_upd'];

        $result_time =   $rating_update[$weight];
        if ($weight_upd==0) $result_time = 86400*30;
        if (!$result_time)$result_time = 86400*30;
...
public static function get_current_meta($table,$id,$aid='aid')
{
    ...
    public static function create_slug($string, $glue = '-') {
...
private function check_slug($name,$count=0)
{
    $regv='#(.+)-([0-9]+)#';
    if (preg_match($regv,$name,$match))
    {
        $name = $match[1];
        $count =  $match[2];
        $count+=1;
    ...
    $q="SELECT `id` FROM `data_actors_imdb` WHERE `slug` =?";
    $r = Pdo_an::db_results_array($q,[$resname]);
    if ($r)
    {
        $count+=1;
        return self::check_slug($name,$count);
    }
...
$name_decoded = self::create_slug($name);
$name_decoded  = self::check_slug($name_decoded);
  ...
  self::var_dump_table(  ['enable'=>$enable,'name'=>$name_db]) ;
    ...
    if (!$enable) {
        ...
        else
        {
            ...
            $sql =  "INSERT INTO `data_actors_imdb`(`id`, `imdb_id` ,`name`, `slug`, `lastupdate`) VALUES (?,?,?,?,?)";
            Pdo_an::db_results_array($sql, array($id,$id, $name,$slug,0));

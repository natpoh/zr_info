    ...
    if (sizeof($data) && isset($data['result']) && sizeof($data['result'])) {
        ...
        <html>
            <head>
                ...
                <title>Nginx access time by domain</title>
            </head>
            <body>
                                <p>
                    <a href="index.php">Access</a> | 
                    <a href="slow.php">Slow</a> | 
                    <b><a href="time.php">Time</a></b> | 
                    <a href="details.php">Details</a> 
                </p>
                <h1>Nginx access time by domain. From <?php print date('d.m.Y H:i:s', $data['start']) ?> to <?php print date('d.m.Y H:i:s', $data['end']) ?></h1>
                <?php
                if (sizeof($result) > 0) {
                    ksort($result);
                    ?>
...
function getLogData($log) {
    ...
    if ($f) {
        ...
        if (fseek($f, 0, SEEK_END) == 0) {//в конец файла -1 символ перевода строки
            ...
            $offset = -2;
            $max_len = 5000000; //Сколько символов читать

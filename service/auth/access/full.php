    ...
    if (sizeof($data)) {
        ...
        foreach ($data as &$item) {
            ...
            if (!$unique_ip[$ip]) {
                ...
                $unique_ip[$ip]['request_total'] = $item['request_time'];
                $unique_ip[$ip]['request_time'] = $item['request_time'];
                $unique_ip[$ip]['request'] = replace_long_text($item['request'], $longtext);
                $unique_ip[$ip]['http_referer'] = replace_long_text($item['http_referer'], $longtext);
                $unique_ip[$ip]['time_local'] = date('H:i:s', strtotime($item['time_local']));
                $unique_ip[$ip]['status_all'] = $item['status'];
            } else {
                $unique_ip[$ip]['count'] +=1;
                $unique_ip[$ip]['bytes_sent'] += $item['bytes_sent'];
                $unique_ip[$ip]['request_total'] += $item['request_time'];
                $unique_ip[$ip]['request_time'] .= "<br />" . $item['request_time'];
                $unique_ip[$ip]['request'].= "<br />" . replace_long_text($item['request'], $longtext);
                $unique_ip[$ip]['http_referer'].= "<br />" . replace_long_text($item['http_referer'], $longtext);
                $unique_ip[$ip]['time_local'].= "<br />" . date('H:i:s', strtotime($item['time_local']));
                $unique_ip[$ip]['status_all'] .= "<br />" . $item['status'];
                $unique_ip[$ip]['request_length'] .= "<br />" . $item['request_length'];
                $unique_ip[$ip]['bytes_sent'] .= "<br />" . $item['bytes_sent'];
            }
        }
        ...
        <html>
            <head>
                ...
                <title>Total online full log</title>
            </head>
            <body>
                    ...
                    <tbody>
                        ...
                        foreach ($data as $ip) {
                            ...
                            <tr>
                                ...
                                <td><?php print replace_long_text($ip['http_user_agent'], 50) ?></td>
                                <td><?php print $ip['request_total']; ?></td>  
                                <td><?php print $ip['status_all']; ?></td>
                                <td><span class="word"><?php print $ip['time_local'] ?></span></td>
                                <td><span class="word"><?php print $ip['request_time'] ?></span></td>
                                <td><span class="word"><?php print $ip['http_referer'] ?></span></td>                        
                                <td><span class="word"><?php print $ip['request'] ?></span></td>
